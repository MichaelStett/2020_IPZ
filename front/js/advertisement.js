...
const advertisements = [
    ...
    new Advertisement('Advertisement8', 3, 'Designed and crafted in Italy. Crystal-tempered sunglass lenses provide protection with distortion-free vision. With a brand character that can best be described as classy, exclusive, stylish, and unique, SUNNY continues to go beyond trends.', more, "./img/ads/Sunglasses_Ad.png", null, Tag.HOT),
    new Advertisement('Advertisement10', 4, 'This windproof, automatic opening recycled umbrella has a vented overlapping canopy, black rubber finished handle and carbon fibre ribs. 90 cm long. Canopy span 120 cm.', more, "./img/ads/Umbrella_Ad.png", null, Tag.RAIN),
];
...
function getTag(data) {
    ...
    if (currentWeather.temperature.current <= 10) {
        tags.push(Tag.COLD);
    }
    if (currentWeather.temperature.current > 10) {
        tags.push(Tag.HOT);
...
function displayAd(weather) {
    ...
    adElements.forEach((i) => {
        ...
        i.append(createAdvertisementHTML(ad(getTag(data))));
    })
}
